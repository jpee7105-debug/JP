...
- **Key Features**:
    ...
    - **Connections (Situation Room)**: Military-themed situation room with canvas-based multi-entity graph (cases as diamonds, people as smaller circles), static layout (no physics), drag-to-move with localStorage position persistence, typed relationship edges (family edges dashed), toggle filters for people/family edges, Graph/Family Tree/Timeline view modes. Family tree mode centers on a person and shows hierarchical family layout.
